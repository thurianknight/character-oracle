...
# Description:
#   1. Copies the character-oracle folders and files from the root folder to the release folder.
#   2. Zips the character-oracle folder and copies it to the release folder.
...
source_root = Path("../character-oracle")
dest = Path("../character-oracle_release/character-oracle")
# base_name = dest / "character-oracle"
character_oracle_module = json.load(open('module.json', 'r'))
...
# Tag the repo in git
version = character_oracle_module['version']
tag = f"v{version}"
...
repo = g.get_repo("thurianknight/character-oracle")

release_name = f"Release {tag}"
release = repo.create_git_release(tag=tag, name=release_name, message=f"Pre-release of character-oracle version {version}", draft=False, prerelease=True)
...
# Upload character-oracle.zip
zip_path = dest.with_suffix('.zip')
release.upload_asset(str(zip_path), label="character-oracle.zip", content_type="application/zip")
print("Uploaded character-oracle.zip to github release.")
